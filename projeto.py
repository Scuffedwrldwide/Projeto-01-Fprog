...
def insere_espacos(cad, col):
    ...
    if cad.find(' ') == False:         #Não encontrar espaços implica a existência de uma só palavra 
        ...
        if len(cad) > col:
            cad = cad.replace(' '*count, ' '*(count-1))        #Reduz ambas as sequências em um espaço
            cad = cad.replace(' '*(count-1), ' '*(count), 1)   #permitindo que se aumente apenas uma das sequencias por 1 espaço
