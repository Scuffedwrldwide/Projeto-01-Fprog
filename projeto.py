...
def justifica_texto(cad, col):
    ...
    if type(cad) != str or type(col) != int or col < 1 or (cad.find(' ') == -1 and len(cad)>col) or len(cad) == 0:
        raise ValueError('justifica_texto: argumentos invalidos')              #Argumentos de tipo errado, cadeia de caracteres nula ou ininterruptamente superior à largura de coluna
    else:
        ...
        cad = []
        for i in range(lines-1):                                               #Itera sobre cada linha, excepto a ultima, que requere um procedimento particular   
